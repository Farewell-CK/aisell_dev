...
from utils.db_queries import select_knowledge, select_product, select_ai_data

async def generate_customer_maintenance_message(tenant_id: str, task_id: str) -> Tuple[str, List[str]]:
    """
    生成一条客情维护话术
    ...
    try:
        ...
        # 构建提示词
        prompt = _build_customer_maintenance_prompt(company_info, product_info, ai_data)
        ...
        return "error", [f"生成客情维护话术失败: {str(e)}"]

def _build_customer_maintenance_prompt(company_info: List[dict], product_info: List[dict], ai_data: List[dict]) -> str:
    """
    构建客情维护提示词，只生成一句话
    """
    # 格式化公司信息
...
### **角色与目标**
你是一个专业的AI销售文案专家。
你的核心任务是根据用户提供的公司及产品资料，生成一句高度精准的微信客情维护话术，目标是让客户知道你是做什么业务的，有需要可以找你。
...
2.  **设计客情维护方案**：自行定义客情维护应遵循的核心原则（例如：价值导向、痛点聚焦、建立信任、避免销售感等），设计一套高成功率的客情维护方案。该方案由两部分构成：
    *   **话术文本**:用于直接发送的文字。
    *   **客情维护资料**: 如果有【可发给客户的资料清单】，则判断是否需要发送资料，需要则从中选择最合适的一个。
    *   **设计注意一**: 如果不知道客户名称则采用通用称呼如"您好"；如果不知道自己的微信名称则无需提及。
    *   **设计注意二**: 话术文本中必须包含公司或产品名称。
    *   **设计注意三**: 客情维护话术文本避免使用句号"。"结尾，以避免生硬感。
